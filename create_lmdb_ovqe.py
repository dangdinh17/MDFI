...
parser.add_argument(
    # '--opt_path', type=str, default='YAML-opt.yml',
...
def create_lmdb_for_mfqev2():
    ...
    with open(yml_path, 'r') as fp:
        ...
        QP = fp['QP']
        # enc_type = fp['dataset']['enc_type']
        gt_folder = fp['dataset']['train']['gt_folder']
        lq_folder = fp['dataset']['train']['lq_folder']
        pd_folder = fp['dataset']['train']['pd_folder']

        gt_path = os.path.join(fp['dataset']['train']['gt_path'])
        lq_path = os.path.join(f'QP{QP}', fp['dataset']['train']['lq_path'])
        # pd_path = os.path.join(f'QP{QP}', fp['dataset']['train']['pd_path'])

        
    gt_dir = op.join(root_dir, gt_folder)
    lq_dir = op.join(root_dir, lq_folder)
    # pd_dir = op.join(root_dir, pd_folder)
    ...
    #     lq_dir,
    #     os.path.basename(gt_video_path)
    #     ) for gt_video_path in gt_video_list]
    # pd_video_list = [op.join(
    #     pd_dir,
    #     os.path.basename(gt_video_path)
    #     ) for gt_video_path in gt_video_list]
    # print(lq_video_list[1])
    print(lq_dir)
    # print(pd_dir)
    ...
    frm_list = []
    for gt_video_path in lq_video_list:
        nfs = int(gt_video_path.split('.')[-2].split('/')[-1].split('_')[-1])
        nfs = nfs if nfs <= 2400 else 2400  # !!!!!!
    ...
    for iter_vid in range(len(lq_video_list)):
        ...
        for iter_frm in range(len(frms)):
            ...
            video_path_list.extend([lq_video_list[iter_vid]]* (2 * radius+1))
            video_lq_list.extend([lq_video_list[iter_vid]] * (2 * radius+1))
            # video_pd_list.extend([pd_video_list[iter_vid]] * (2 * radius+1))

            # video_mvr_list.extend([mvr_video_list[iter_vid]] * (2 * radius+1))
            # video_mvg_list.extend([mvg_video_list[iter_vid]] * (2 * radius+1))
            # video_mvb_list.extend([mvb_video_list[iter_vid]] * (2 * radius+1))
            # video_pm_list.extend([pm_video_list[iter_vid]] * (2 * radius+1))
            # video_rm_list.extend([rm_video_list[iter_vid]] * (2 * radius+1))
    ...
    # print("Writing LMDB for RM data...")
    # make_y_lmdb_from_yuv(
    #     video_path_list=video_rm_list,
    #     index_frame_list=index_frame_list,
    #     key_list=key_list,
    #     lmdb_path=lmdb_rm_path,
    #     multiprocessing_read=True,
    #     )
    ...
    # print("Writing LMDB for PD data...")
    # make_y_lmdb_from_yuv(
    #     video_path_list=video_pd_list,
    #     index_frame_list=index_frame_list,
    #     key_list=key_list,
    #     lmdb_path=lmdb_pd_path,
    #     multiprocessing_read=True,
    #     )
    # print("> Finish.")
