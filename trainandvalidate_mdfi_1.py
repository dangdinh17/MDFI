...
def receive_arg():
    ...
    parser.add_argument(
        '--opt_path', type=str, default='configs/train_MDFI_1.yml',
...
def main():
    ...
    # ==========
    model = MDFI_1(**opts_dict['network']['hyper'])
    # model = OVQE()
    model = model.to(rank)
    if opts_dict['train']['is_dist']:
    ...
    # Visual prompt and freezing
    # ovqe_state = torch.load(opts_dict['train']['load_best_ovqe'])['state_dict']
    # mdfi_state = model.state_dict()
    # pretrained_dict = {k: v for k, v in ovqe_state.items() if k in mdfi_state and v.size() == mdfi_state[k].size()}
    # mdfi_state.update(pretrained_dict)
    # model.load_state_dict(mdfi_state)

    # print(f"Loaded {len(pretrained_dict)} layers from total {len(ovqe_state)} in OVQE -> MDFI")
    # for name, param in model.named_parameters():
    #     if name in pretrained_dict:   # chỉ freeze các layer được load thành công
    ...
    best_ssim = -float('inf')
    # load checkpoint
    opts_dict['train']['checkpoint'] = last_path if opts_dict['train']['checkpoint'] == "None" else opts_dict['train']['checkpoint']
    if os.path.exists(opts_dict['train']['checkpoint']):
        ...
        print(f"Resume from iteration: {start_iter} with best psnr {best_psnr} and best ssim {best_ssim}")

    # # # Freeze tất cả các layer
    # for param in model.parameters():
    #     param.requires_grad = False

    # # Chỉ bật train cho STFF_1
    # for param in model.stff.parameters():
    ...
    for current_epoch in range(start_epoch, num_epoch + 1):
        ...
        while train_data is not None:
            ...
            if ((num_iter_accum % interval_val == 0) or \
                ...
                loss_item = loss.item()
                avg_psnr, avg_ssim = utils.validate(model, val_gt, val_lr, val_pd)
                if using_comet:
