...
class ResumeDistSampler(Sampler):
    """Distributed Sampler with resume support.
    
    Args:
        dataset (Dataset): dataset to sample from.
        num_replicas (int): number of processes (world_size).
        rank (int): rank of current process.
        ratio (int): enlarging ratio. Default: 1.
        start_iter (int): iteration to resume from inside current epoch. Default: 0.
        batch_size (int): batch size per process. Default: 1.
    """
    def __init__(self, dataset, num_replicas=None, rank=None, ratio=1, start_iter=0, batch_size=1):
        self.dataset = dataset
        self.num_replicas = num_replicas
        self.rank = rank
        self.epoch = 0
    ...
    def __iter__(self):
        ...
        # ==== NEW: skip indices that were already processed ====
        start_index = self.start_iter * self.batch_size
        if start_index < len(indices):
            indices = indices[start_index:]
        else:
            indices = []  # resume beyond dataset size → empty

        return iter(indices)

    def __len__(self):
        # giảm độ dài theo phần skip
        start_index = self.start_iter * self.batch_size
