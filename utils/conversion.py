...
def paired_random_crop_prior_res(img_gts, img_lqs, img_pds, gt_patch_size, gt_path, scale=1):
    """Paired random crop.
    It crops lists of lq and gt images with corresponding locations.
    Args:
        img_gts (list[ndarray] | ndarray): GT images. Note that all images
    ...
    if not isinstance(img_lqs, list):
        img_lqs = [img_lqs]
    if not isinstance(img_pds, list):
        img_pds = [img_pds]
    ...
    img_pds = [ v[top:top + lq_patch_size, left:left + lq_patch_size, ...] for v in img_pds ]
    # img_pms = [ v[top:top + lq_patch_size, left:left + lq_patch_size, ...] for v in img_pms ]
    # img_mvs = [ v[top:top + lq_patch_size, left:left + lq_patch_size, ...] for v in img_mvs ]
    # img_rms = [ v[top:top + lq_patch_size, left:left + lq_patch_size, ...] for v in img_rms ]
    ...
    # if len(img_rms) == 1:
    #     img_rms = img_rms[0]
    return img_gts, img_lqs, img_pds
